  ...
  jQuery('.drawer-icon').on('click', function(e){
    // eventを持つことができる
    e.preventDefault();
    // aタグで別のページに飛んでしまうのを無効化できる
    // clickイベントを使うときはセットで使うといいかも
    // alert('clickされました')で確認できます
  
    jQuery('.drawer-icon').toggleClass('is-active');
    jQuery('.drawer-content').toggleClass('is-active');
    jQuery('.drawer-background').toggleClass('is-active');
    return false;
    // 古いブラウザ用ですが慣例としてつけておく　なくてもOK
  ...
  jQuery(window).on('scroll', function(){
    ...
    if(680 < jQuery(this).scrollTop()) {
      jQuery('.subheader').addClass('is-fixed');
  
    } else{
      jQuery('.subheader').removeClass('is-fixed');
...
jQuery('a[href^="#"]').on('click', function(){
  ...
  var subheader = jQuery('.subheader').innerHeight();
  var id = jQuery(this).attr('href');

  var position = 0;
  if( id != '#'){
position = jQuery(id).offset().top - subheader;
  }
